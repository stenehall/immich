...
describe('AssetService', () => {
  ...
  describe('getRange', () => {
    it('should work with start and end in header', async () => {
      const rangeRequestHeader = 'bytes=0-1023';
      const range = sut.getRange(rangeRequestHeader, 1024);
      expect(range).toStrictEqual({ start: 0, end: 1023 });
    });

    it('should work with only start in header', async () => {
      const rangeRequestHeader = 'bytes=0-';
      const range = sut.getRange(rangeRequestHeader, 1024);
      expect(range).toStrictEqual({ start: 0, end: 1023 });
    });

    it('should work with only end in header', async () => {
      const rangeRequestHeader = 'bytes=-1023';
      const range = sut.getRange(rangeRequestHeader, 1024);
      expect(range).toStrictEqual({ start: 0, end: 1023 });
    });

    it('should work with partial start and end range', async () => {
      const rangeRequestHeader = 'bytes=128-255';
      const range = sut.getRange(rangeRequestHeader, 1024);
      expect(range).toStrictEqual({ start: 128, end: 255 });
    });

    it('should work with partial start range', async () => {
      const rangeRequestHeader = 'bytes=128-';
      const range = sut.getRange(rangeRequestHeader, 1024);
      expect(range).toStrictEqual({ start: 128, end: 1023 });
    });

    it('should work with partial end range', async () => {
      const rangeRequestHeader = 'bytes=-255';
      const range = sut.getRange(rangeRequestHeader, 1024);
      expect(range).toStrictEqual({ start: 0, end: 255 });
    });

    it('should return empty object if no range header', async () => {
